...
if file is not None:
    file_wav = librosa.load(file)# return signal elements and sampling freqS
...
with remove_button_col:
    remove_signal_button = st.button("Remove", key="remove_button", disabled=len(
        functions.get_Total_signal_list()) <= 0)
    if remove_signal_button:
...
functions.sinGeneration(signal_amplitude_slider, signal_frequancy_slider, signal_phase_slider)
functions.generateFinalSignal(noise_flag, file_as_array, SNR_slider_value)
fig1,fig2,fig3, Reconstructed_signal = functions.renderSampledSignal(
